...
class DslBuilder(
    ...
    private fun generateProperty(builder: StringBuilder, parameter: KSValueParameter) {
        ...
        if (isNullable || hasDefaultValue) {
            ...
            builder.appendLine("var $name: $typeName${if (isNullable) "?" else ""} = ${getDefaultValueForType(type)}")
        } else {
            // For required non-nullable parameters, use Delegates.notNull()
            builder.appendLine("var $name: $typeName by Delegates.notNull()")
        }
    }
        ...
        return when (type.nullability) {
            Nullability.NULLABLE -> "null"
                ...
                when {
                    ...
                    typeName.startsWith("kotlin.collections.") -> "emptyList()"
                    typeName.startsWith("kotlin.Array") -> "emptyArray()"
                    typeName.startsWith("kotlin.Map") -> "emptyMap()"
                    typeName.startsWith("kotlin.Set") -> "emptySet()"
                    else -> "null as $typeName" // This is a fallback and might not compile
                }
            }
        }
    }
