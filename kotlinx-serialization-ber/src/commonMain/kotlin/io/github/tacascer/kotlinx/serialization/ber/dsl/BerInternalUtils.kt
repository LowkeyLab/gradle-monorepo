...
import io.github.tacascer.kotlinx.serialization.ber.BerWriter
...
internal object BerInternalUtils {
    ...
    fun encodeBerElement(tagClass: BerTagClass, tag: BerTag, content: ByteArray): ByteArray {
        ...
        // Write length
        if (content.size < 128) {
            writer.writeByte(content.size.toByte())
        } else {
            // Write long form length
            val lengthBytes = encodeLengthBytes(content.size)
            writer.writeByte((lengthBytes.size or 0x80).toByte())
    ...
    fun encodeBerElement(
        ...
        writer.writeByte(tagByte.toByte())
